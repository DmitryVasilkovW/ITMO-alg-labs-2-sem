...
n математиков и трое программистов собираются ехать на поезде в другой город на конференцию. Они встречаются у кассы на вокзале. Первой подходит очередь программистов, и они, как все нормальные люди, покупают по билету на человека. Математики же покупают один билет на всех.
...
В первой строке находятся два числа n и k (1 ≤ n ≤ 5⋅10^5, 1 ≤ k ≤ 10^6) - количество математиков и максимальная граница запросов. Так как каждый математик делает по одному запросу, количество математиков равноценно количеству запросов.

В следующих n строках записаны по два числа a_i и b_i - запрос i-того математика (1 ≤ a_i, b_i ≤ k).

Выходные данные

В ответ на каждый запрос с новой строки выведите количество простых чисел в промежутке между a_i и b_i включительно.
...
В первой строке записаны два числа n и m - количество вершин и ребёр в графе (0 < m < n < 10^5). В следующих 
m строках записаны по два числа u и v, которые означают, что вершины под номерами u и v соединены ребром.

Выходные данные

В единственной строке выведите YES, если в графе есть Гамильтонов Путь, или NO в противном случае.

# Sort Me advertisement

Почему Sort Me так быстро и уверенно развивается? Секрет прост: они особым образом выбирают телеграм-чаты для рекламы.

Обычно происходит так: главе Sort Me приносят список из n чатов, где в i-том чате a_i участников. Глава выбирает три таких чата под индексами i, j и k, что 
i < j < k и a_i ⊕ a_j ⊕ a_k = 0 (где ⊕ - операция побитового исключающего ИЛИ). Такая тройка чатов считается идеальным набором чатов для рекламы.

Посчитайте, сколько у главы способов выбрать идеальный набор из заданных n чатов.

Входные данные

В первой строке находится число n (1 ≤ n ≤ 2⋅10^5) – количество чатов, которое в очередной раз принесли главе Sort Me.

В следующей строке записано n чисел a_1 ,a_2 , …, a_n−1, a_n, где a_i – количество участников в i-том чате. (
1 ≤ a_i ≤ 10^4).
...
# Create coca-cola tree

Пракилоп закупил для своей новогодней вечеринки n баночек с колой. Чтобы впечатлить гостей, он решил не просто оставить их на столе, а построить из них ёлку!

Известно, что ёлка – это конструкция, в которой на каждом уровне размещено хотя бы на одну баночку больше, чем на уровне выше.

Посчитайте, ёлку какой максимальной высоты можно собрать из n баночек. Разумеется, часть баночек можно не использовать и просто оставить на столе.

Входные данные

В единственной строке записано число n (0 ≤ n ≤ 10^19) - количество баночек с колой.

Выходные данные

Выведите одно число - максимальную высоту ёлки, которую можно составить из n баночек колы.

# Crazy number system

В попытках впечатлить учителя информатики Лиза придумала собственную систему счисления. В этой системе есть 
n разрядов, каждый из который может быть установлен в состояние 0 или 1. Алгоритм перевода из Лизиной системы в десятичную прост: пробежимся по всем разрядам с конца, и для каждого 
i-го разряда со значением 1 добавим к ответу натуральное число a_i.
...
На утро вы аккуратно, не будя Лизу, пошли к компьютеру и решили всё-таки проверить, насколько её учитель прав. Найдите самый маленький натуральный 
x, который нельзя записать в Лизиной системе.

Входные данные

В первой строке записано натуральное число 
n (1 ≤ n ≤ 2⋅10^5) – количество разрядов в Лизиной системе.

В следующей строке записаны числа a_1, a_2 ... a_n
(1 ≤ a_i ≤ 10^9).

Выходные данные

Выведите одно число x – самое маленькое натуральное число, которое нельзя записать в Лизиной системе счисления.

# Commemorative coins

В одном из карманов вашего друга всегда лежит только бесконечное количество юбилейных трёхрублёвых монет, а в другом – бесконечное количество юбилейных пятирублёвых.
...
Помогите другу посчитать, какими монетами ему рассчитаться за товар, чтобы не получить сдачу, и отдать кассиру минимально возможное число монет.

Входные данные

В единственной строке записано число x – стоимость товара (3 ≤ x ≤ 10^9).

Выходные данные

В единственной строке выведите два числа через пробел – минимальное число трёхрублёвых и пятирублёвых монет, которое друг отдаст на кассе.
